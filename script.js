...
$(document).ready(function() {
	...
	$("#all").on("click", function() {
		filter ("#all", "#online", "#offline", "tr[data-status='online']", "", "tr[data-status='offline']");
	});
	$("#online").on("click", function() {
		filter ("#online", "#all", "#offline", "tr[data-status='online']", "tr[data-status='offline']", "");
	});
	$("#offline").on("click", function() {
		filter ("#offline", "#all", "#online", "tr[data-status='offline']", "tr[data-status='online']", "");
	});
...
function getData () {
	...
	users.forEach(function(user) {
		...
		for (var i = 0; i < twitch.length; i++) {
			if 	(!!twitch[i]["stream"]) {
				if  (user === twitch[i]["stream"]["display_name"])  {
					$("tbody").append(
						code('online" class = "online', twitch[i]["_links"]["self"], twitch[i]["stream"]["logo"], twitch[i]["stream"]["display_name"], twitch[i]["stream"]["status"])
					);
					check = true;
				}
			} else if (user === twitch[i]["display_name"]) {
					$("tbody").append(
						code ('offline', twitch[i]["_links"]["self"], twitch[i]["_links"]["logo"], twitch[i]["display_name"], 'Offline')
					);
					check = true;
			}
		}
		if (!!!check) {
			notthere = notthere + user + ", ";
		}
	});
	$("table").fadeIn(2000, function () {
		alert("The following users do not exist: " + notthere.slice(0, notthere.length - 2));
	});
}

function code(stat, linkself, linklogo, name, activity) {
	return '<tr data-status = "' + stat + '"><td><a target = "_blank" href="' + linkself + '"><img src="' + linklogo + '"><span>' + name + '</span></a></td><td>' + activity + '</td></tr>';
}

function filter(bon1, bout1, bout2, ain1, aout1, ain2) {
	$(bon1).addClass("active");
	$(bout1).removeClass("active");
	$(bout2).removeClass("active");
	$(ain1).fadeIn(750);
	$(aout1).fadeOut(750);
	$(ain2).fadeIn(750);
}
